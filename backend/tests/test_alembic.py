...
def test_head_revision_is_the_ingestion_runs_table_migration() -> None:
    """STORY-015 chained a new migration onto STORY-014's — the head moved
    again. (This is the fourth time this exact test has needed updating —
    STORY-009→010, STORY-010→011, STORY-011→014, now STORY-014→015 — a
    cheap, mechanical adjustment each time. Worth generalizing in a future
    Story if this keeps recurring, but not part of STORY-015's approved
    scope to refactor now.)"""
    script = ScriptDirectory.from_config(_alembic_config())
    head = script.get_revision(script.get_heads()[0])
    assert head.down_revision is not None
    assert "ingestion_runs" in (head.doc or "").lower()
