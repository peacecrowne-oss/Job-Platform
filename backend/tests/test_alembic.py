...
def test_head_revision_is_the_add_job_closed_at_migration() -> None:
    """STORY-028 chained a new migration onto STORY-021's — the head moved
    again. (This is the seventh time this exact test has needed updating —
    STORY-009→010, STORY-010→011, STORY-011→014, STORY-014→015,
    STORY-015→057, STORY-057→021, now STORY-021→028 — a cheap, mechanical
    adjustment each time. Worth generalizing in a future Story if this
    keeps recurring, but not part of STORY-028's approved scope to
    refactor now.)"""
    script = ScriptDirectory.from_config(_alembic_config())
    head = script.get_revision(script.get_heads()[0])
    assert head.down_revision is not None
    assert "add job closed_at and source freshness threshold" in (head.doc or "").lower()
